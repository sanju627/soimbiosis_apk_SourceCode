...
public class Item_BTN : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        int totalSRV = database.srvAmount + database.srvRomanLegion + database.srvRomanCaeser + database.srvRedFox + database.srvBiden + database.srvTrump + database.srvSoldierBlue + database.srvSoldierRed + database.srvSTPMale + database.srvSTPFemale;

        if (totalSRV > 0 && item.IsWeapon() && !item.IsPistol() && weaponManger.withinStorage)
        {
            SetBTNOBJ.SetActive(true);
        }
        else
